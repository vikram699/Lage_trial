...
public class TC_EndToEnd extends BaseClass{
	...
	@Test(dataProvider = "getWebData", testName = "TC_EndToEnd")
	public void completeEndToEnd(Hashtable<String,String> data) throws Exception {
		if (!data.get("RunMode").equalsIgnoreCase("N")){
			...
			page
			.setBrowserStackCap(data, "End To End")
			.launchInstance(data.get("browser"));
			page
			.openApplication(data.get("PocUrl"))
			.getLogin(data)
			.goToHome();
			page
			.goToSales("Dashboard")
			.goToJobs("DashBoard");
			page
			.selectJobTicket(data.get("SelectRegion"))
			.updateTicketandCreate("1", "1");
			data = page.returnTicketNumber(data); 

			page.createNewWindow(1) 
			.openApplication(data.get("Url"))
			.getLogin(data)
			.postLogin(data);

			fo = new FODashBoardPage(Constants.BadgerObjectRepository, node);
			fo.setDriver(page.getDriver());
			...
			data = fo.verifyAssignedJob(data, fo.dragFirstJob(data));
			fo.verifyJobAndChangeTime(data, "12:00PM");
			fo.setUpRoute(data);
			fo
			.acknowledgements()
			.Notes()
